...
import math
import urllib.request  # ← MJPEG HTTP 파싱에 사용
...
class GUI(QMainWindow):
    def __init__(self, ros: RosBridge, image_topics: dict[str, str]):
        ...
        left_layout.addWidget(self.pose_label, 2, 0)

        # ✅ MJPEG(HTTP) 파싱용 버퍼 & 연결
        self._http_buf = bytearray()
        self._http_conn = None

        # JetCobot 업데이트 타이머
        self.jet_timer = QTimer()
        self.jet_timer.timeout.connect(self.update_jetcobot_stream_http)
        self.jet_timer.start(33)  # ~30fps
    ...
    # ── JetCobot 스트림 표시 (HTTP로 MJPEG 직접 파싱)
    def update_jetcobot_stream_http(self):
        try:
            # 연결 없으면 접속 시도 (매 주기마다 1s timeout)
            if self._http_conn is None:
                req = urllib.request.Request(
                    JETCOBOT_STREAM_URL,
                    headers={'User-Agent': 'Mozilla/5.0'}
                )
                self._http_conn = urllib.request.urlopen(req, timeout=1.0)

            # 데이터 조금씩 읽어서 JPEG 프레임 추출
            chunk = self._http_conn.read(4096)
            if chunk:
                self._http_buf += chunk
                while True:
                    soi = self._http_buf.find(b'\xff\xd8')  # JPEG SOI
                    eoi = self._http_buf.find(b'\xff\xd9')  # JPEG EOI
                    if soi != -1 and eoi != -1 and eoi > soi:
                        jpg = self._http_buf[soi:eoi+2]
                        del self._http_buf[:eoi+2]
                ...
                if self.image_jetcobot.pixmap() is None:
                    self.image_jetcobot.setText("JetCobot 스트림 대기중…")

        except Exception:
            # 오류 시 연결 리셋 → 다음 주기에 재연결
            try:
                if self._http_conn:
                ...
                pass
            self._http_conn = None
            self._http_buf.clear()
            self.image_jetcobot.setText("JetCobot 연결 재시도…")
