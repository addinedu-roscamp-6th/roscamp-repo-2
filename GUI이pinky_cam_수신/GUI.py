...
import math
import cv2  # Added for HTTP stream capture
...
# ===== 비디오/포즈 토픽 설정 =====
IMAGE_TOPICS = {
    "Global":   "/marker/debug_image",  # Will be overridden by HTTP stream
...
HTTP_STREAM_URL = "http://192.168.4.1:8889"  # URL for the HTTP stream from cam_send_IP.py
...
class RosBridge(Node):
    ...
    def __init__(self, image_topics: dict[str, str], pose_topics: dict[str, str]):
        ...
        for name, topic in image_topics.items():
            if name != "Global":  # Skip ROS2 subscription for Global (use HTTP stream instead)
...
class GUI(QMainWindow):
    def __init__(self, ros: RosBridge, image_topics: dict[str, str]):
        ...
        self.video_capture = cv2.VideoCapture(HTTP_STREAM_URL)  # Initialize HTTP stream capture
        ...
        for (row, col), name in zip(positions, self.image_names):
            box = QGroupBox(f"{name} ({HTTP_STREAM_URL if name == 'Global' else image_topics[name]})")
        ...
        self.log_action(f"GUI 시작. HTTP 스트림 ({HTTP_STREAM_URL}) 및 다중 카메라/포즈 구독; /pinkyN/go_to_*, /pinkyN/stop 퍼블리시")
        if not self.video_capture.isOpened():
            self.log_action(f"HTTP 스트림 연결 실패: {HTTP_STREAM_URL}")

    # 타이머 루프
    def _on_timer(self):
        rclpy.spin_once(self.ros, timeout_sec=0.0)
        # Update ROS2-based camera feeds
        for name, qimg in self.ros.latest_qimages.items():
            if name == "Global":  # Skip Global (handled by HTTP stream)
                continue
        ...
        if self.video_capture.isOpened():
            ...
            if ret:
                ...
                qimg = QImage(cv_rgb.tobytes(), w, h, bytes_per_line, QImage.Format.Format_RGB888).copy()
                pix = QPixmap.fromImage(qimg.scaled(CELL_W, CELL_H, Qt.AspectRatioMode.KeepAspectRatio))
                self.video_labels["Global"].setPixmap(pix)
            else:
                self.ros._log_gui("HTTP 스트림 프레임 읽기 실패")
        else:
            self.ros._log_gui("HTTP 스트림 연결 끊김")
    ...
    def closeEvent(self, event):
        try:
            self.video_capture.release()  # Release HTTP stream capture
