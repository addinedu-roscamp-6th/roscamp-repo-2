...
OBSTACLE_DISTANCE = 0.5       # 장애물 감지 거리 (m)
...
class ControllerNode(Node):
    ...
    def control_loop(self):
        ...
        if distance < GOAL_TOLERANCE:
            ...
            if self.ranges and min(self.ranges[len(self.ranges)//2 - 10: len(self.ranges)//2 + 10]) < OBSTACLE_DISTANCE:
                self.get_logger().info("[OBSTACLE] Detected, rotating to avoid")
                cmd.linear.x = 0.0
                cmd.angular.z = ROTATE_SPEED  # 오른쪽으로 회전
            else:
