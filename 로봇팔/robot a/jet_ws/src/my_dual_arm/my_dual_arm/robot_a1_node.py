...
class RobotA1Node(Node):
    def __init__(self):
        ...
        self.action_map = {
            ...
            107: [0, -15, -80, 8, 6, 45, 0],
            108: [0, -15, -80, 8, 6, 45, 100],
            ...
            115: [0, -10, -75, 0, 6, 45, 0],
            116: [0, -10, -75, 0, 6, 45, 100],
            117: [0, 0, -70, -20, 0, 45, 100],

            201: [0, -10, -45, -35, 6, -45, 100],
            202: [0, -10, -75, 0, -60, 45, 100],
            203: [0, -10, -75, 0, 6, 45, 100],
            204: [0, -10, -75, 0, 6, 45, 0],
            205: [0, 0, -40, -48, 6, 45, 0],
            206: [0, 40, -130, 0, 6, 45, 0],
            207: [0, 40, -130, 0, 6, 45, 100],
            208: [0, -15, -80, 8, -60, 45, 100],
            209: [0, -15, -80, 8, 6, 45, 100],
            210: [0, -15, -80, 8, 6, 45, 0],
    ...
    def _camera_loop(self):
        ...
        while True:
            ...
            cv2.drawMarker(fr, (w//2, h//2), (0,255,0), cv2.MARKER_CROSS, 20, 2)
            with self._marker_lock:
                stable_sec = time.time() - self._marker_anchor_ts if (self._marker_seen and self._marker_anchor_ts is not None) else 0.0
    ...
    def to_marker_basic(self):
        ...
        if not ok:
            raise RuntimeError("마커 안정 대기(0.5s) 실패")
        ...
        self._wait_if_paused()
        self._ckpt_send_coords(target_coords.tolist() + ori, speed=self.SLOW_SPEED, mode=self.BLEND_MODE); time.sleep(5)
        ...
        try:
            ...
            if angles is None:
                ...
                self._wait_if_paused()
                self._ckpt_send_angle(6, j6_temp, self.SLOW_SPEED); time.sleep(1.2)

                self._wait_if_paused()
                self._ckpt_set_gripper(0, 50); time.sleep(0.5)

                self._wait_if_paused()
                self._ckpt_send_angle(6, j6_saved, self.SLOW_SPEED); time.sleep(1.0)
        except Exception as e:
            self.get_logger().warn(f"J6 일시 회전/복귀 처리 실패: {e}")

        try:
            self._wait_if_paused()
            final_angles = [0, 45, -125, -10, 6, -45]
            self._ckpt_send_angles(final_angles, self.FAST_SPEED); time.sleep(0.7)

            self._wait_if_paused()
            self._ckpt_send_angle(5, 0, self.FAST_SPEED); time.sleep(0.5)

            self._wait_if_paused()
            self._ckpt_set_gripper(100, 50); time.sleep(0.5)

            self._wait_if_paused()
            self._ckpt_send_angle(6, 45, self.FAST_SPEED); time.sleep(0.5)

            self._wait_if_paused()
            self._ckpt_send_angle(5, -60, self.FAST_SPEED); time.sleep(0.5)
    ...
    def run_code1_feature(self):
        ...
        self._wait_if_paused()
        self._ckpt_set_gripper(100, 50); time.sleep(0.3)
        self._wait_if_paused()
        ...
        try:
            ...
            while True:
                try:
                    ...
                    self.step_done = False
                    self.get_logger().error(f"❌ Step {step} (CODE1) 실패: {e} → 재시도합니다.")
                    self._wait_if_paused()
                    time.sleep(0.5)
        finally:
            ...
            if self.step_done:
                ...
                self.get_logger().warn(f"🚫 Step {step} (CODE1) 미완료: DONE 미발행")
        ...
        try:
            ...
            if step in (101, 201):
                ...
                while True:
                    ...
                    ok = self.wait_marker_stable(still_time=0.5, pos_eps=None, timeout=15.0)
                    if ok:
                        self.get_logger().info(f"✅ Step {step}: 마커 0.5s 안정 확인")
                        break
                    else:
                        self.get_logger().warn(f"⏳ Step {step}: 마커 안정 타임아웃 → 계속 대기")
                        time.sleep(0.3)
            ...
            if self.step_done:
                ...
                self.get_logger().warn(f"🚫 Step {step} 미완료: DONE 미발행")
